...
SELECT ecoclassid_std, ecoclassname, ecotype, (ST_Area(SHAPE)/10000) AS area_ha, (ecoarea_sqm/ST_Area(SHAPE)) AS ecopct, ST_Multi(SHAPE) AS SHAPE
  FROM (
       SELECT ST_Union(SHAPE) AS SHAPE, ecoclassid_std, ecoclassname, ecotype, sum(ecoarea_sqm) AS ecoarea_sqm
         FROM (
              SELECT a.SHAPE, b.ecoclassid_std, b.ecoclassname, b.ecotype, 
                     (ST_Area(a.SHAPE)*(CAST(b.ecoclasspct AS REAL)/100)) AS ecoarea_sqm
                FROM mupolygon AS a
