...
SELECT group_cols, ecoclassid_std, ecoclassname, ecotype, area_ha, ecoarea_ha/area_ha AS ecopct, ST_ForcePolygonCW(ST_MakeValid(geom)) AS geom
...
SELECT ecoclassid_std, ecoclassname, ecotype, area_ha, ecoarea_ha/area_ha AS ecopct, ST_ForcePolygonCW(ST_MakeValid(geom)) AS geom
  FROM area_calc_union
...
SELECT a.ecoclassid_std, a.ecoclassname, a.ecotype, a.area_ha, a.ecopct, b.range_prod, b.forest_prod,
       d.habitat veg_type, c.mlra, a.geom 
  FROM make_valid AS a
  LEFT JOIN prod AS b ON a.ecoclassid_std = b.ecoclassid_std
  LEFT JOIN group_mlra AS c ON a.ecoclassid_std = c.ecoclassid_std
...
SELECT ecoclassid_std, ecoclassname, ecotype, area_ha, ecopct, range_prod, forest_prod,
       veg_type, mlra, geom
