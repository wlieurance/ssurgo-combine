...
CREATE VIEW IF NOT EXISTS {schema}.chorizon_surface AS
SELECT * 
FROM chorizon 
WHERE hzdept_r = 0;""",

"""/* This selects the top layer FROM {schema}.chorizon that is not organic material */
CREATE VIEW IF NOT EXISTS {schema}.chorizon_surface_noOM AS
SELECT *, min(hzdept_r)
...
CREATE VIEW IF NOT EXISTS {schema}.chunified_first AS
...
(in order of ecoclasstype abundance within the database). JOIN {schema}.to component or qry_component_dominant via cokey. */
CREATE VIEW IF NOT EXISTS {schema}.coecoclass_codominant AS
SELECT y.*, 
...
CREATE VIEW IF NOT EXISTS {schema}.coecoclass_mapunit_ranked AS
...
CREATE VIEW IF NOT EXISTS {schema}.coecoclass_mudominant AS
SELECT mukey, ecoclassid, ecoclassid_std, ecoclassname, ecoclassname_std, ecotype, MAX(ecoclasspct) AS ecoclasspct
  FROM (SELECT * FROM {schema}.coecoclass_mapunit_ranked ORDER BY mukey, ecoclasspct DESC, ecoclassid)
 GROUP BY mukey;""",
...
CREATE VIEW IF NOT EXISTS {schema}.coecoclass_wide AS
SELECT a.mukey, 
  ...
  LEFT JOIN (
       SELECT mukey, ecoclassid, ecoclassname, ecoclasspct 
         FROM (
              SELECT mukey, ecoclassid, ecoclassname, ecoclasspct, 
                    (SELECT COUNT(*) 
                       FROM {schema}.coecoclass_mapunit_ranked AS b 
...
CREATE VIEW IF NOT EXISTS {schema}.coecoclass_plantprod AS
...
CREATE VIEW IF NOT EXISTS {schema}.comonth_flodpondfreqcl AS
SELECT a.cokey, 
...
"""/* Provides minimum and maximum soil moisture depth levels group by comonth.  JOIN {schema}.to component or qry_component_dominant */
CREATE VIEW IF NOT EXISTS {schema}.comonth_soimoistdept AS
...
CREATE VIEW IF NOT EXISTS {schema}.component_mapunit_ranked AS
SELECT a.mukey, a.cokey, a.compname, 
  ...
  FROM (
       SELECT mukey, cokey, compname, comppct_r, localphase, slope_l, slope_h
         ...
         LEFT JOIN (SELECT * FROM {schema}.chorizon WHERE hzdept_r = 0) as b ON a.cokey = b.cokey
         LEFT JOIN {schema}.chtexturegrp AS c ON b.chkey = c.chkey;""",
...
CREATE VIEW IF NOT EXISTS {schema}.component_dominant AS
SELECT mukey, cokey, compname, compnamelong, MAX(comppct_r) AS comppct_r
  FROM (SELECT * FROM {schema}.component_mapunit_ranked ORDER BY mukey, comppct_r DESC, compname) AS a 
 GROUP BY mukey;""",
 
"""/* Isolates the six most dominant components per mapunit and arranges them in a wide table format with their percentages
Also renames components by adding local phase, surface texture, and slope range.  JOIN {schema}.to mupolygon or mapunit via mukey.
This is a really inefficient query :( */
CREATE VIEW IF NOT EXISTS {schema}.component_wide AS
SELECT x.mukey,
       ...
       SELECT * 
         FROM (
              SELECT mukey, cokey, compname, compnamelong, comppct_r, 
                    (SELECT COUNT(*) 
                       FROM {schema}.component_mapunit_ranked AS b 
                      WHERE b.mukey = a.mukey AND 
...
Join to chorizon or qry_chorizon_surface via chkey. */
CREATE VIEW IF NOT EXISTS {schema}.texture_first AS
SELECT a.*, c.texcl, c.lieutex, c.chtkey FROM {schema}.chtexturegrp AS a
INNER JOIN (
...
"""/* Compiles a list of unique components in the database and displays some statistics, including total area in hectares
   Calculated FROM {schema}.the mupolygon feature and the comppct_r field. */
CREATE VIEW IF NOT EXISTS {schema}.component_unique AS
